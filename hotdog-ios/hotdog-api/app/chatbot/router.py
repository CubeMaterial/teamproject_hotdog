...
def fallback_answer_from_records(source: str, records: list[dict]) -> str:
    if source == "buy":
        lines = ["구매 내역을 확인했어요."]
        for record in records[:3]:
            name = record.get("product_name") or "상품"
            date = record.get("buy_date")
            quantity = record.get("buy_qty")
            price = record.get("buy_price")
            detail = " · ".join(str(value) for value in [date, f"{quantity}개" if quantity else None, f"{price}원" if price else None] if value)
    ...
    if source == "deliver":
        ...
        for record in records[:3]:
            name = record.get("product_name") or "상품"
            start = record.get("deliver_start_date")
            end = record.get("deliver_end_date")
            detail = " · ".join(str(value) for value in [start, end] if value)
            lines.append(f"- {name}" + (f" ({detail})" if detail else ""))
        return "\n".join(lines)

    lines = ["확인된 상품 기준으로 추천드릴게요."]
    for record in records[:5]:
        name = record.get("product_name") or record.get("name") or "상품"
        category = record.get("product_sub_category_name") or record.get("product_category_name")
        price = record.get("product_price")
        stock = record.get("product_qty")
        detail = " · ".join(
            str(value)
            for value in [
                category,
                f"{price}원" if price else None,
                f"재고 {stock}개" if stock is not None else None,
            ]
            if value
        )
        lines.append(f"- {name}" + (f" ({detail})" if detail else ""))
...
def chat_message(request: MessageRequest):
    ...
    try:
        ...
        answer = fallback_answer_from_records(database_context.source, database_context.records)
        append_exchange(session_id, message, answer)
        return api_response(
            True,
            "chat response generated without ollama",
            {
                "answer": answer,
                "session_id": session_id,
                "base_url": settings.base_url,
